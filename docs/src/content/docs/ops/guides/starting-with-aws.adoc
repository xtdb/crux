...
See link:https://docs.aws.amazon.com/signin/latest/userguide/command-line-sign-in.html[**AWS CLI Sign-In Instructions**^] for more information on how to authenticate the AWS CLI - for this guide, we will use `aws sso`:
...
NOTE: We will later create an IAM role that can be assumed by the EKS cluster to access the S3 bucket via the command line - you may wish to add this to the Terraform configuration.
...
* You may also wish to change resource tiers, the location for the resources to be deployed on, or the VM sizes used by the Google Cloud cluster.
...
=== Setting up an EBS Storage Class

In order for us to persist data on the Kafka nodes, we need to set up a storage class on the cluster that we can use to create persistent volumes.

For our purposes, we will set up a storage class that uses the `gp3` EBS volume type.
...
Second, apply the storage class to the cluster:

```bash
kubectl apply -f storage-class.yaml
```

There are further notes around this available in the link:https://docs.aws.amazon.com/eks/latest/userguide/create-storage-class.html[**Amazon EKS Storage Class Documentation**^].
...
helm install kafka oci://registry-1.docker.io/bitnamicharts/kafka \
  ...
  --set global.defaultStorageClass=gp3-ebs \
...
```bash
aws iam create-role --role-name xtdb-eks-role --assume-role-policy-document file://eks_policy_document.json
```
...
kubectl annotate serviceaccount xtdb-service-account \
  --namespace xtdb-deployment \
   eks.amazonaws.com/role-arn=<xtdb_eks_role_arn>
```
...
```bash
curl -X POST http://$ExternalIP:8080/healthz/alive
